#!/usr/bin/env node
/*
  prepend-thumbnail.js

  Description:
  - Creates a short thumbnail video from an image and prepends it to a short video.
  - Final output is always an .mp4 file.
  - Logs and saves a copy of the config (with history) to the configured path.

  Usage:
    node prepend-thumbnail.js path/to/config.js

  NOTE: The config file can be a JS module (recommended, allows comments and trailing commas)
        or a JSON file. The user requested "Not use json format" — this script prefers a
        JS module config (e.g. export default { ... } or module.exports = { ... }).

  The script will display the FFmpeg commands it executes and will ask the user
  interactively if the final output file exists (Overwrite? [y/N]).
*/
