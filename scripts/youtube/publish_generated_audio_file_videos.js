(async function () {
  ...
  async function getElementByPlaceholder(
    tag,
    placeholderText,
    retries = 5,
    delay = CONFIG.GLOGAL_DELAY,
  ) {
    ...
    throw new Error(
      `Element with placeholder containing "${placeholderText}" not found after ${retries} retries.`,
    );
  }

  async function getElementContainingText({
    tag,
    text,
    retries = 5,
    delay = CONFIG.GLOGAL_DELAY,
    elementToQueryIn = document,
  }) {
    for (let i = 0; i < retries; i++) {
    ...
    throw new Error(
      `Element with direct text "${text}" not found after ${retries} retries.`,
    );
  }
  ...
  async function getElementByAriaLabelContains({
    ...
    throw new Error(
      `Element with aria-label containing "${text}" not found after ${retries} retries.`,
    );
  }
  ...
  async function getPrivateRadioButtonLabel({
    retries = 5,
    delay = CONFIG.GLOGAL_DELAY,
  } = {}) {
    for (let i = 0; i < retries; i++) {
      ...
      if (matches.length === 1) {
        ...
        throw new Error(
          'Multiple tp-yt-paper-radio-button elements contain "Private".',
        );
      }
    ...
    throw new Error(
      'No tp-yt-paper-radio-button element contains "Private" after retries.',
    );
  }

  async function mainFlow({ videoToPublish }) {
    const convertedVideoName = normalizeFilename(
      videoToPublish.originalVideoName,
    );
    console.log("convertedVideoName: ", convertedVideoName);

    const draftAnchorTag = await getElementContainingText({
      tag: "a",
      text: convertedVideoName,
    });
    draftAnchorTag.click();

    await waitMilliseconds(700);

    const reuseDetailsButton = await getElementContainingText({
      tag: "div",
      text: "Reuse details",
    });
    ...
    const searchYourVideosInput = await getElementByPlaceholder(
      "input",
      "Search your videos",
    );
    ...
    const previousVideoToSelect = await getElementContainingText({
      tag: "div",
      text: stringToSearch,
    });
    ...
    const modalTitle = await getElementContainingText({
      tag: "h2",
      text: "Select the details you would like to reuse",
    });
    const searchDialog = modalTitle.closest("tp-yt-paper-dialog");

    const reuseButtonInDialog = await getElementContainingText({
      tag: "div",
      text: "Reuse",
      elementToQueryIn: searchDialog,
    });
    ...
    const titleCustomInput = await getElementByAriaLabelContains({
      text: "Add a title that describes your video",
      elementToQueryIn: videoDetailsModal,
    });

    await simulateTypingInCustomInput({
      element: titleCustomInput,
      text: incrementParteNumber(stringToSearch),
      delay: 5,
    });

    const nextButton = await getElementContainingText({
      tag: "div",
      text: "Next",
      elementToQueryIn: videoDetailsModal,
    });
    ...
    const nextButton2 = await getElementContainingText({
      tag: "div",
      text: "Next",
      elementToQueryIn: videoDetailsModal,
    });
    ...
    const nextButton3 = await getElementContainingText({
      tag: "div",
      text: "Next",
      elementToQueryIn: videoDetailsModal,
    });
    ...
    const saveButton = await getElementContainingText({
      tag: "div",
      text: "Save",
      elementToQueryIn: videoDetailsModal,
    });
  ...
  const videosToPublish = [
    {
      latestAudioTitleToBasedOn: "Ghost Recon: Wildlands - Parte 42",
      originalVideoName:
        "ghost_recon_wildlands_2025-09-13 12-24-01-EDITED--PROCESSED--AUDIO-GAME.mp4",
    },
    {
      latestAudioTitleToBasedOn: "Ghost Recon: Wildlands - Parte 42",
      originalVideoName:
        "ghost_recon_wildlands_2025-09-13 12-24-01-EDITED--PROCESSED--AUDIO-MIXED.mp4",
    },
    {
      latestAudioTitleToBasedOn: "Ghost Recon: Wildlands - Parte 42",
      originalVideoName:
        "ghost_recon_wildlands_2025-09-13 12-24-01-EDITED--PROCESSED--AUDIO-VOICE1.mp4",
    },
    {
      latestAudioTitleToBasedOn: "Ghost Recon: Wildlands - Parte 42",
      originalVideoName:
        "ghost_recon_wildlands_2025-09-13 12-24-01-EDITED--PROCESSED--AUDIO-VOICE2.mp4",
    },
  ];
